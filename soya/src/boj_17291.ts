...
function solution() {
  ...
  let stack_year: number[] = [0, 1, 0, 0, 0]; // 0 1 2 3 4
  ...
  for (let i = 2; i <= n; i++) {
    ...
    if (i % 2 === 0) {
      stack_year[0] = newBugs; // 분열해서 생긴애들은 올해 탄생한 애들. 짝수년도면 0칸에 저장. 4회때죽으니까. 덮어쓰기.
    } else if (i % 2 !== 0) {
      stack_year[1] += newBugs; // 홀수년도면 1칸에 저장. 3회때죽으니까. 작년짝수년도애들에 더해줌.
      stack_year[0] = 0;
    }
